...
class OnlineDeepSearch(DeepSearch):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.depth_assessor = information_depth.InformationDepthAssessor(self.llm, self.embedding_model)
    ...
    def _assess_knowledge_gaps_for_query(
            self,
            original_query: str,
            current_query: str,
            local_results_for_query: List[RetrievalResult],
            all_sub_queries: List[str],
            global_state: dict
    ) -> Tuple[dict, int]:
        """
        Assess knowledge gaps for a specific query, based only on local search results for that query
        """

        # Only use local results for the current query for evaluation
        comprehensive_assessment, assessment_tokens = self._assess_comprehensive_knowledge_gaps(
            original_query,
            [current_query],  # Evaluate only the current query
            local_results_for_query,  # Use only local results for the current query
            global_state
        )

        # If there are no or few local results, force a network search
        if len(local_results_for_query) == 0:
            comprehensive_assessment["needs_web_search"] = True
            comprehensive_assessment["confidence_score"] = 0.0
            comprehensive_assessment["knowledge_gaps"].append(f"No local results found for query: {current_query}")
            comprehensive_assessment[
                "reasoning"] = f"No local knowledge found for '{current_query}', web search required."
        elif len(local_results_for_query) < 2:
            # Too few results, lowering the confidence score
            comprehensive_assessment["confidence_score"] = min(comprehensive_assessment["confidence_score"], 0.3)
            comprehensive_assessment["needs_web_search"] = True
            comprehensive_assessment[
                "reasoning"] += f" Very few local results ({len(local_results_for_query)}) for '{current_query}'."

        return comprehensive_assessment, assessment_tokens
    ...
    def _update_global_state_with_analysis(self, global_state: dict, topic_analyses: Dict[str, information_depth.TopicAnalysis]):
        ...
        # Initialize information_depth if not exists
        if "information_depth" not in global_state:
            global_state["information_depth"] = {}
        ...
        for topic, analysis in topic_analyses.items():
            # Preserve existing evidence_count if exists
            existing_evidence = global_state["information_depth"].get(topic, {}).get("evidence_count", 0)
            ...
            global_state["information_depth"][topic] = {
                ...
                "evidence_count": existing_evidence + analysis.evidence_count,
    ...
    def _update_global_knowledge_state(self, state: dict, query: str, results: List[RetrievalResult], source_type: str):
        """Updates the global knowledge state with consistent data structure"""
        if not results:
            return
        ...
        # Initialize information_depth structure if not exists
        if "information_depth" not in state:
            state["information_depth"] = {}

        # Update information depth for each topic
        for topic in query_topics:
            # Get current depth info or initialize default structure
            current_info = state["information_depth"].get(topic, {
                "overall_score": 0,
                "coverage": 0,
                "quality": 0,
                "authority": 0,
            ...
            # Calculate new evidence count (simple count of matching results)
            new_evidence = len([r for r in results if topic.lower() in r.text.lower()])

            # Update depth information while preserving other metrics
            state["information_depth"][topic] = {
                "overall_score": current_info.get("overall_score", 0),
                "coverage": max(current_info.get("coverage", 0), min(new_evidence / 5, 1.0)),
                "quality": current_info.get("quality", 0),
                "authority": current_info.get("authority", 0),
                "diversity": current_info.get("diversity", 0),
                "depth": max(current_info.get("depth", 0), min(new_evidence / 3, 1.0)),
                "coherence": current_info.get("coherence", 0),
                "evidence_count": current_info.get("evidence_count", 0) + new_evidence,
                "source_diversity": current_info.get("source_diversity", 0)
            }
    ...
    async def _execute_batch_web_search(self, web_queries: List[str]) -> List[RetrievalResult]:
        ...
        try:
            ...
            if filtered_urls:
                ...
                # Batch load all URLs
                from deepsearcher.offline_loading import load_from_website,create_safe_metadata
                ...
                collection_names = [col.collection_name for col in self.vector_db.list_collections() if hasattr(col, 'collection_name')]
                log.color_print(
                    f"<debug_collections_list> Collections available in vector_db BEFORE check: {collection_names} </debug_collections_list>\n")
                #
                # Retrieving content from a temporary collection
                if temp_collection in collection_names:
    ...
    def _generate_query_specific_summary(self, results: List[RetrievalResult]) -> str:
        """enerate a summary of search results for a specific query"""
        if not results:
            return "No local information retrieved for this query"

        summary_parts = []
        summary_parts.append(f"Local Knowledge ({len(results)} chunks):")

        # Take the first 3 results as samples
        for i, result in enumerate(results[:3], 1):
            sample_text = result.text[:150] + "..." if len(result.text) > 150 else result.text
            summary_parts.append(f"  {i}. {sample_text}")

        if len(results) > 3:
            summary_parts.append(f"  ... and {len(results) - 3} more chunks")
    ...
    async def async_retrieve(
        ...
        min_info_gain_threshold = kwargs.pop("min_info_gain", self.min_info_gain)
        ...
        global_knowledge_state = {
            ...
            "assessment_summary": {
                "total_topics": 0,
                "well_covered_topics": 0,
                "partially_covered_topics": 0,
                "poorly_covered_topics": 0,
        ...
        for iter in range(max_iter):
            ...
            if not sub_gap_queries:
                ...
                for query_idx, query in enumerate(sub_gap_queries, 1):
                    ...
                    local_search_results, consumed_token = await self._search_chunks_from_vectordb(query, [query])
                    ...
                    log.color_print(
                        f"<local_search_result> Retrieved {len(local_search_results)} chunks from local knowledge base for '{query}' </local_search_result>\n")
                    ...
                    if self.enable_web_search and query_web_searches < max_web_searches_per_think:
                        comprehensive_assessment, assessment_tokens = self._assess_knowledge_gaps_for_query(
                            original_query,
                            query,
                            local_search_results,
                            all_sub_queries,
                        ...
                        if (comprehensive_assessment.get("needs_web_search", False) and
                            ...
                            candidate_web_queries, query_tokens = self._generate_web_search_queries(
                                ...
                                self._generate_query_specific_summary(local_search_results)
                            )
                            ...
                            if filtered_web_queries:
                                ...
                                current_iteration_results.extend(local_search_results)
